...
如果输出里 `java.project.getAll` 或 `java.project.listSourcePaths` 是 0，说明 jdtls 虽然启动了，但 Maven/Gradle 项目没有成功导入；通常需要补齐公司 parent POM、私服配置或本地依赖缓存。`Maven environment probe` 会检查 `mvn`、`settings.xml`、本地仓库、mirror/profile/repository 数量但不输出账号密码或私服 URL；`Maven parent probe` 会继续检查最近 `pom.xml` 的 parent 链，并指出缺失的 GAV、相对路径和 `~/.m2` 路径。LCA 会继续用 lightweight fallback 给出类/方法定位；当你传入明确类名或符号名时，fallback 会优先扫描文件名/路径匹配的候选文件，但这仍不是完整 type-aware navigation。
