...
| 风险 | R-004 | 中 | 中断时 tool_calls 配对仍可增强 | 已缓解 | deadline 和用户中断已补齐；输出截断场景后续处理 |
| 风险 | R-005 | 中 | 没有初始 git commit | 已关闭 | 已创建初始 commit |
| 风险 | R-006 | 低 | 高级能力过早引入 | 受控 | P6 暂缓，先稳定日用闭环 |
| 风险 | R-007 | 中 | Prompt injection | 开放 | 文档提示；不信任仓库禁用 yolo |
| 风险 | R-008 | 中 | P3/P4 变更尚未提交 | 已关闭 | P3 提交 `304fbdf`，P4 提交 `4beb487` |
| 风险 | R-009 | 中 | ask_user 会阻塞等待用户 | 开放 | 长任务需求尽量写清；后续评估 ask_user 超时策略 |
| ADR | ADR-001 | 2026-07-07 | 不照搬 OMP，只借鉴能力类型和边界 | 已接受 | 每个能力做简化版 |
| ADR | ADR-002 | 2026-07-07 | max_steps 只作为防失控保险丝 | 已落地 | 默认值已改为 0，不限步 |
| ADR | ADR-003 | 2026-07-07 | todo、ask_user、per-tool approval 是主功能 | 已落地 | P3 已实现 |
| ADR | ADR-004 | 2026-07-07 | 第一阶段 memory 用 Markdown | 已接受 | 后续看需求升级 |
| ADR | ADR-005 | 2026-07-07 | Patch 先用 anchored patch，不上 AST edit | 已接受 | P5 再评估 preview/rollback |
| ADR | ADR-006 | 2026-07-07 | 长需求建议放文件让 Agent read_file | 已接受 | README 已写推荐工作流 |
| ADR | ADR-007 | 2026-07-07 | 封闭 VM 下不做公网搜索/自动下载 | 已接受 | 依赖提前准备 |
| ADR | ADR-008 | 2026-07-07 | Excel 给人看，Markdown 给 Agent 读 | 已接受 | 持续同步本文件和 `project-status.md` |
| ADR | ADR-009 | 2026-07-07 | OMP 核心架构笔记单独固化 | 已接受 | 见 `docs/omp-core-architecture-notes.md` |
