...
| 测试数 | 141 | 完整 unittest、compileall、diff check、xlsx 检查通过。 |
...
| Multi-root workspace | 已完成 MVP 版 | `--allow-dir` / `AGENT_ALLOWED_DIRS` 支持显式授权额外目录给文件、搜索、LSP、patch 工具；system prompt 会列出 primary workspace 和 allowed dirs；shell/git/显式项目 memory/skills 仍锚定 `--cwd`，session/todo/patch logs 和默认 consolidation memory 走 state dir。 |
...
| Multi-root workspace | 已完成 MVP 版 | `--allow-dir` / `AGENT_ALLOWED_DIRS` | 支持需求文档目录 + 代码项目目录；system prompt 会列出 allowed dirs，文件/search/LSP/patch 可访问 allowed dirs，shell/git/显式项目 memory/skills 仍锚定 `--cwd`，默认 consolidation memory 走 state dir | 真实需求压测 |
...
| T-057 | P0 | P7 | allowed-dir workspace roots 注入 | 已完成 MVP 版 | Agent | session `20260708T065705459243Z` 暴露模型不知道 `--allow-dir` 具体路径，误猜 `requirements` 目录 | system prompt/provider-bound context 增加 `[Workspace roots]`，列出 primary `--cwd` 和 allowed dirs；file tool descriptions 同步说明 allowed directory；测试覆盖不重复注入 |
...
| 风险 | R-020 | 高 | multi-root allowed dir 没有显式进入模型上下文 | 已关闭 MVP 版 | session `20260708T065705459243Z` 中模型不知道需求目录绝对路径，尝试 `requirements` 并失败，导致未读需求文档就反推代码 | OMP 会显式提供 cwd/project context/rules；我们新增 `[Workspace roots]`，把 primary `--cwd` 和 allowed dirs 写入启动 system prompt 与 provider-bound context。 |
...
| PT-011 | P0 | 已关闭 MVP 版，待用户复跑验证 | `--allow-dir` 具体路径没有进入模型上下文，模型猜 `requirements` 目录失败后没有读取真实需求文档。 | OMP 会把 cwd/project context 和工具可用范围显式放进运行上下文。 | 已补 `[Workspace roots]` 注入和 file tool 描述；下一次同命令应先 list/read allowed-dir 绝对路径。 |
