...
独立 review 发现第一版 Git metadata contract 仍有两个边界缺口：实现/设计/概念解释只要提到 Git repository 就可能被降为 metadata 只读任务；任意非空 `git_probe_root` 也可能被当作 primary，且 final 文本的“是/不是 Git 仓库”没有与 `git_repository` 布尔事实核对。`c682b7b` 将 metadata contract 限为直接的 current/primary workspace Git 状态问句，并以 `task_kind=read-only`、scope、direct-question 共同约束；审计只接受 `evidence_root_label=primary`，要求 final Git 结论与 structured probe 极性一致。empty/refusal semantic answer 也不能再仅凭 inspection-forbidden contract 通过。

本次按本机 OMP 源码重新对照：`packages/coding-agent/src/session/tool-choice-queue.ts` 将 directive 的 in-flight / resolve / reject / `not_invoked` 生命周期作为 queue owner；`packages/coding-agent/src/session/agent-session.ts` 在 `turn_end` 统一 resolve/reject，并把 active tools 投影到 agent；`packages/agent/src/agent-loop.ts` 在逻辑 turn 内区分 soft requirement 与 hard choice。LCA 因单 Runtime 裁剪不复制该队列实现，但遵循同一 ownership 原则：先确认 task kind，再挂 Git metadata owner；`CompletionAudit` 消费 typed `GitRepositoryObservation(subject, root_label, repository, provenance)`，而不是由最终文字反推工具事实。final 极性核对是 LCA 附加、有限的审计，不声称 OMP 有同名 parser。新增 unit/runtime 回归后，完整基线为 **542/542**；未重复发送 live 请求，T-147 继续等待独立 review。

第二轮 review 再收紧 direct-status classifier：metadata owner 现在只接受完整 primary/workspace Git yes/no 命题（例如“当前 primary 是否/是不是 Git 仓库”或英文 copular 等价）；裸 `?` 不再充分。因此“检测函数如何实现”“检测逻辑如何实现”“为何不是 Git 仓库”等保留原 task contract，不会被 metadata probe 抢占；无问号的“确认当前 primary 是不是 Git 仓库”仍覆盖。提交 `76bdc8e`；未重跑 live。
