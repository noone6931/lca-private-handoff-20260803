...
当前 deterministic gate 为 1192/1192 tests、62/62 benchmark、25/25 architecture checks；T-241 stable release 为 `20260718T031104Z-ebc2f5040cc3-8d06c53405c8`，revision `ebc2f5040cc30e6e58ed0bb37a0fc33935ce85e9`，digest `8d06c53405c84c952d4aa3508a9e724f636272a2e915c2590a8712b87ea0059a`。publish gate 在 clean detached candidate 中用 Python 3.14 再次通过 unittest、compileall 和 diff-check；独立 matrix 还覆盖普通 coding continuity、semantic preview/jdtls 边界、full-screen TUI reducer/mailbox/interaction/cooperative cancellation、PTY restore/signal、`/workspace list` completion，以及 bracketed paste 和 wrapped-row scrollback。TUI 继续由单一 UI 线程投影和写屏，worker 只分发 typed command；`lca --tui "task"` 的初始任务也只经 worker 提交一次。T-241 新增 64 KiB 原子 bracketed paste、PageUp/PageDown、mouse/trackpad wheel、resize-safe history anchor 和 `history x-y/n` footer，并修正 macOS ncurses Button5 fallback 与整数 `KEY_ENTER` 下的 Alt-Enter 兼容。确定性 wheel owner 与 Page PTY 已通过，自动化环境中的真实物理滚轮 PTY 仍为 INCONCLUSIVE；完整 async command bus、并发 turn、remote UI、OS sandbox 和并发/写入 subagent 仍未实现，也不冒充追平 Codex/OMP。
