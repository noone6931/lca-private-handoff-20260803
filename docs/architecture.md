...
| Isolated read-only reviewer / ExploreHandoff | `[MVP-已落地，live provider 待复测]` | `explore_handoff.py`、`read_only_reviewer.py`、`runtime_read_only_review.py`、`read_only_explore.py`、`task_contract.py`、RunSummary。 | 高风险只读 owner/impact/design contract 先产生 typed profile；primary candidate final 前，Runtime 至多发起一次独立 `tools=[]` reviewer operation。候选 Markdown 会以最多 80 个稳定 claim units 做全段均衡采样；schema 失败返回仅含 code/shape/count 的脱敏诊断，最多两次 repair、合计三次 reviewer provider calls，绝不接受非法数据。reviewer finding 必须返回唯一 `claim_id`；Runtime 在一次无工具改写后检查原 claim unit 是否以 Markdown-normalized exact 形式回流。合法 `unverified` finding也获得这一次诚实降级改写；未知 ID、协议/超时、预算不足或回流仍以未验证终止。typed profile 的探索仅允许精确 read/search/LSP；每个代码 root 有 bounded read 或达到 hard budget 后强制候选进入 reviewer。 | 借鉴 OMP `tools/yield.ts` 的有界 schema retry、`task/executor.ts` 的 soft request budget 和 `prompts/agents/reviewer.md` 的 typed yield/output schema。LCA 不复制 OMP task platform，也不采用其 schema override：repair 耗尽必定 `unverified`。 |
