...
| Isolated read-only reviewer / ExploreHandoff | `[WIP-增强中]` | `explore_handoff.py`、`read_only_reviewer.py`、`runtime_read_only_review.py`、`read_only_explore.py`、`runtime_read_only_explore.py`、`steering/pre_review.py`、`task_contract.py`、RunSummary。 | 高风险只读 owner/impact/design contract 先产生 typed profile；reviewer 只通过 isolated output-only `submit_read_only_review` 提交结构化结论，不进入 ToolRegistry、approval 或工作区执行。T-158~T-187 已把 claim locator/binding、incremental yield、finding replay、document conflict context、transport recovery、rewrite 和 safe-partial 的 queued/in-flight/resolved/rejected 生命周期分别固化；同一 transport recovery 最多一次，耗尽失败关闭。 | 借鉴 OMP `tool-choice-queue.ts` 的 directive 生命周期、`agent-loop.ts` 的 turn boundary 与 `yield.ts` 的结构化输出；LCA 的 document stance/reviewer 是本地证据增强，不宣称 OMP 有同名 taxonomy。真实 provider 仍会返回 malformed final、在 final-only schema 下继续调用已关闭 output tool，或让 reviewer 自创技术方案，因此当前只算 candidate 能力，不能替换 stable。 |
...
当前 deterministic gate 为 830 tests、35/35 benchmark、7/7 architecture checks。immutable candidate 为 `20260714T170102Z-c248d3598dc2-6f417eb9e525`；默认 stable 仍为 T-149。T-190 live gate 只有 S1 natural final，S2/S3 均由 provider adapter 识别普通文本中的工具协议 envelope 并以 `provider_protocol_violation` 终止，证明“失败关闭”有效但尚不具备日用发布质量。T-191 二次运行没有复现协议违规，却暴露两个更稳定的 owner 缺口：`read_only_explore.py` 在首次无可信候选搜索后未进入一次 root-scoped fallback discovery，`task_contract.py` 又没有把通用“证据化技术设计/设计建议”投影为 typed `design` profile，使 S3 绕过 isolated reviewer。这两个问题分别归 explore/ToolChoiceQueue 和 contract/reviewer lifecycle；不应改成 provider 文本 scrubber，也不应在 `agent.py` 增加自然语言 guard。OMP 对照点是 `prompts/agents/explore.md` 的空搜索 alternate strategy、`session/tool-choice-queue.ts` 的 directive resolve/reject owner，以及 `task/agents.ts` + `tools/yield.ts` 的 typed role/output lifecycle。发布条件不是“安全拒绝即可”，而是母需求 S1~S3 在 fresh state 下同时达到有界工具调用、自然 final、证据/建议分层和 reviewer 不引入无证据方案，并取得独立黑盒回归。
