...
| Session Evidence Continuity / User Facts | `[MVP-已落地]` | `session_evidence.py`、`runtime_evidence.py`、`user_facts.py`、`evidence.py`、RunSummary。 | 对齐 OMP session tool-result continuity：同一 Runtime 仅复用 fresh positive concrete-path read/search/LSP evidence，投影前逐路径 hash；negative/incomplete/global evidence 不跨轮复用。重复观测按 canonical path + query/range 替换。命名 session 跨进程恢复只接受重新从磁盘校验并重建的正向 `read_file` 证据；JSONL 中的 content/search/LSP payload 不被信任。缓存命中后 ToolChoiceQueue 只发一次 soft directive，不移除 `read_file` schema；模型复读仍正常执行并单列 telemetry。 | write/rollback、workspace revision/root change、`/move` 或外部内容变化会失效；summary 以 `role=user` + `attribution=runtime` 发送，不能把混合摘要提升为 system。T-151 后 `agent.py` 为 1,897 行、74 个方法，final-answer facade 为 59 行；此前 2,494 行是 T-127 历史节点。 |
...
| Isolated read-only reviewer / ExploreHandoff | `[MVP-已落地，live provider 兼容性待收束]` | `explore_handoff.py`、`read_only_reviewer.py`、`runtime_read_only_review.py`、`task_contract.py`、RunSummary。 | 高风险只读 owner/impact/design contract 先产生 typed profile；primary candidate final 前，Runtime 至多发起一次独立 `tools=[]` reviewer call。候选 Markdown 会以最多 80 个稳定 claim units 做全段均衡采样，reviewer finding 必须返回唯一 `claim_id`；Runtime 在一次无工具改写后检查原 claim unit 是否以 Markdown-normalized exact 形式回流。合法 `unverified` finding也获得这一次诚实降级改写；未知 ID、协议/超时、预算不足或回流仍以未验证终止。 | 借鉴 OMP `prompts/agents/explore.md` / `reviewer.md` 的隔离只读上下文与结构输出、`task/{types,executor,discovery,yield-assembly}.ts` 的 task output ownership，以及 `advisor/runtime.ts` 的独立上下文生命周期。LCA 未实现 OMP task worktree、batch、IRC、advisor backlog 或完整 explore subagent；ExploreHandoff 只是现有 evidence 的小型结构化投影，不是第二套搜索引擎。 |
