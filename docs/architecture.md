...
| Isolated read-only reviewer / ExploreHandoff | `[WIP-增强中]` | `explore_handoff.py`、`read_only_reviewer.py`、`runtime_read_only_review.py`、`read_only_explore.py`、`runtime_read_only_explore.py`、`steering/pre_review.py`、`task_contract.py`、RunSummary。 | 高风险只读 owner/impact/design contract 先产生 typed profile；reviewer 只通过 isolated output-only `submit_read_only_review` 提交结构化结论，不进入 ToolRegistry、approval 或工作区执行。T-158~T-187 固化 claim/rewrite/transport/safe-partial 生命周期；T-191 让通用技术设计意图进入 typed `design` profile，并在每个缺少 direct candidate 的 root 首次有效搜索后开放一次 fallback discovery。 | 借鉴 OMP `tool-choice-queue.ts` 的 directive 生命周期、`agent-loop.ts` 的 turn boundary 与 `yield.ts` 的结构化输出；LCA 的 document stance/reviewer 是本地证据增强，不宣称 OMP 有同名 taxonomy。T-191 live 证明工具名限制不足以约束 `glob_files` 参数形状，且两轮 reviewer rewrite 仍可能不收敛；下一步应在 directive/queue 与 reviewer Owner 内补 typed lifecycle，不能改成 Runtime 自动搜索或 provider 文本补丁。 |
...
当前 deterministic gate 为 832 tests、37/37 benchmark、7/7 architecture checks。T-191 提交 `d91cda0`，immutable candidate 为 `20260714T175058Z-d91cda0b2d24-ae8b6f1a1970`；默认 stable 仍为 T-149。T-191 已关闭首次无命中后的 fallback 死区和通用技术设计绕过 reviewer 两个离线缺口；fresh-state S2/S3 都实际进入 isolated reviewer，错误 Owner/现有实现/设计事实没有释放。但两场分别出现 14 和 23 次工具错误：临时 discovery 只投影 `glob_files` 工具名，provider 仍反复生成缺 `paths`、错误 `path/pattern` 或错误 root 的参数；随后 reviewer 三轮均为 revise，rewrite verification 未能关闭同一 finding 集，最终安全未验证。T-192 的 Owner 边界因此限定为 temporary directive/ToolChoiceQueue 的 typed argument intent 与 reviewer rewrite closure。OMP 对照点仍是 `prompts/agents/explore.md` 的 alternate strategy、`session/tool-choice-queue.ts` 的 queued/in-flight/resolve/reject 和 `task/agents.ts` + `tools/yield.ts` 的 typed role/output lifecycle；不得在 `agent.py` 增加 domain guard，也不得让 Runtime 代模型自动执行任意业务搜索。发布条件仍是母需求 S1~S3 在 fresh state 下有界自然收束，并取得独立黑盒回归。
