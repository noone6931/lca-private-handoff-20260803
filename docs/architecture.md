...
| Isolated read-only reviewer / ExploreHandoff | `[MVP-已落地，live provider 待收敛]` | `explore_handoff.py`、`read_only_reviewer.py`、`runtime_read_only_review.py`、`read_only_explore.py`、`runtime_read_only_explore.py`、`task_contract.py`、RunSummary。 | 高风险只读 owner/impact/design contract 先产生 typed profile。T-154 将 reviewer 改为唯一的 output-only `submit_read_only_review` function schema；它不是 ToolRegistry 工作区工具，不经审批、不执行文件或命令。schema 严格限制 verdict/confidence/reason、已知唯一 claim ID、`pass=0` findings、非 pass 为 1..8 条及大小上限；initial 最多 3 次、rewrite review 最多 2 次，repair 只回传脱敏 shape diagnostics，耗尽必定 `unverified`。所有 deterministic final steerer 先收敛，再进入 reviewer；首次 non-pass 只允许一次无工具改写，改写候选重新切分并接受第二次 semantic review，第二次仅 `pass` 才能释放。探索在可信 search/LSP path 出现时立即抑制同批剩余调用，下一 turn 仅投影这些 canonical `read_file` paths；无候选或预算耗尽则 scoped/unlocated。 | 借鉴 OMP `tools/yield.ts` 的唯一 typed yield/schema repair、`task/executor.ts` 的有界 request lifecycle、`prompts/agents/reviewer.md` 的隔离 reviewer output。LCA 不复制 task platform 或 schema override；output tool 不进入 OMP/LCA workspace tool registry，严格失败不会被截断或默许。百炼实际 owner/design 仍会过度探索或给出第二次 non-pass，当前以安全未验证终态收束。 |
