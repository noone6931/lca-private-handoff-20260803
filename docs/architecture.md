...
当前 deterministic gate 为 1100/1100 tests、62/62 benchmark、21/21 architecture checks，另有 14/14 process matrix；T-228 stable release 为 `20260717T081918Z-24d6daa4f827-c67e5ebe043c`，revision `24d6daa4f827e777dcb0b249ab3c066b0e9facb3`，digest `c67e5ebe043c8a52fa50c38fabb1b0cec82c55f6556b5b0361b014ce1c65524c`。publish gate 在 clean detached candidate 中用 Python 3.14 再次通过 unittest、compileall 和 diff-check。该版本继承 T-222 default-off Explore Subagent、T-224 rename preview 与 T-226 Code Action preview，并收住 jdtls Eclipse metadata 副作用；command、server applyEdit、resource operation、非授权路径和 partial invalid preview 仍全部拒绝。async queue、取消、OS sandbox、并发/写入 subagent、worker/runtime 隔离与完整 TUI 仍未实现，不冒充追平 Codex/OMP。
