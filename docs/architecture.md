...
| Isolated read-only reviewer / ExploreHandoff | `[MVP-已落地，live provider 待收敛]` | `explore_handoff.py`、`read_only_reviewer.py`、`runtime_read_only_review.py`、`read_only_explore.py`、`runtime_read_only_explore.py`、`task_contract.py`、RunSummary。 | 高风险只读 owner/impact/design contract 先产生 typed profile；primary candidate final 前，Runtime 至多发起一次独立 `tools=[]` reviewer operation。候选 Markdown 会以最多 80 个稳定 claim units 做全段均衡采样；schema 失败返回仅含 code/shape/count 的脱敏诊断，最多两次 repair、合计三次 reviewer provider calls，绝不接受非法数据。response contract 明确 `<9000` 字符、`pass=0` findings、非 pass 为 1..8 个已知且唯一的 claim ID。reviewer finding 必须返回唯一 `claim_id`；Runtime 在一次无工具改写后检查原 claim unit 是否以 Markdown-normalized exact 形式回流。合法 `unverified` finding也获得这一次诚实降级改写；未知 ID、协议/超时、预算不足或回流仍以未验证终止。typed profile 的探索仅允许精确 read/search/LSP；同一 tool-call batch 达到 hard budget 后其余调用合成 suppressed observation，且已有 typed search/LSP 路径会优先投影为有界 `read_file`，以形成 per-root 直接读取证据。 | 借鉴 OMP `tools/yield.ts` 的有界 schema retry、`task/executor.ts` 的 soft request budget 和 `prompts/agents/reviewer.md` 的 typed yield/output schema。LCA 不复制 OMP task platform，也不采用其 schema override：repair 耗尽必定 `unverified`。 |
