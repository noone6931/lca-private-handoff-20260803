...
| Context / Rules | `[MVP-已落地]` | Workspace roots、用户级/项目级 `AGENTS.md` 启动注入，`RULES.md` 每轮 sticky 注入；multi-root roots 也会进入关键工具观察。项目级 `AGENTS/RULES` 的 canonical target 必须留在 primary workspace，内部 leaf/parent symlink 保留，外部及特殊文件 fail closed；用户级语义不变。 | `src/local_agent/workspace/startup.py` 是 Markdown source/reader Owner；`--cwd`、`--allow-dir`、`~/.config/local-coding-agent/`、`.local-agent/`。这是 prompt-source path containment，不是 OS sandbox。 |
...
| LSP semantic rename preview | `[T-224-已落地]` | 独立 `lsp/workspace_edit.py` 与 `tools/lsp_rename.py` Owner；复用 external LSP client、ToolRegistry 和 workspace roots。 | 外部 LSP 负责 `textDocument/rename`；Phase 1 只接受 text-only WorkspaceEdit，先全量校验 URI、authorized/project roots、UTF-16 range、重叠及文件/编辑/累计输入/输出预算，再在内存生成 bounded diff。preview 零写盘、零 patch log、`evidence_eligible=false`；parent 必须 read 候选文件。真实 jdtls 样本已完成该生命周期。 | T-224 有意只做 preview；T-259 在独立 write-tier tool 中补显式批准的 exact-plan apply，不让 LSP client/server 直接写盘。Codex 的 Tool Router/ExecutionPolicy/approval 用作边界参考；prepareRename、workspace/applyEdit、resource operation 与自动应用仍不支持。 |
| Approved text-only WorkspaceEdit apply | `[T-259-已落地]` | `lsp/workspace_edit_store.py` 独占 bounded immutable plan；`tools/workspace_edit.py` 只做 plan identity/authorization bridge；`patch/transaction.py` 独占 existing-file mutation/compensation；`patch/journal.py` 独占原子 journal persistence。 | Preview 绑定 session/run/canonical workspace/allowed roots/project root/完整 LSP server identity 和 before hashes。`apply_workspace_edit` 是既有 ToolRegistry 的 write tier，只接受 `plan_id`；apply 前不重发 LSP 请求，完整 preflight 后逐文件写入，失败同步补偿并按最终 residual truth 报告。journal temp 完整 flush/fsync 后以单次 `os.replace` commit，失败保持旧 journal；成功 apply/rollback 继续进入既有 attribution、diff 和 rollback handle。 | 对齐 OMP 先验证完整 WorkspaceEdit batch 和 Codex approval/router 分层，但保持更窄的 existing regular UTF-8 TextEdit。Command、edit+command、create/rename/delete、changeAnnotations、server applyEdit 和自动调用 fail closed；不虚称跨文件 crash-atomic，也不增加第二 writer/approval tree。 |
...
当前 deterministic gate 为 1203/1203 tests、62/62 benchmark、25/25 architecture checks；T-243 stable release 为 `20260718T035333Z-4271172e8d36-fab5ff2b17ed`，revision `4271172e8d36c390d512eaded6b1f06052dbd10a`，digest `fab5ff2b17edd4094d8fca3fc9871598997c89110d438af63f2978ac1fda4707`。publish gate 在 clean detached candidate 中用 Python 3.14 再次通过 unittest、compileall 和 diff-check；94 项候选 matrix 与安装后 PTY smoke 覆盖 reducer/mailbox/interaction/cooperative cancellation、`/workspace list`、fragmented UTF-8/CSI、bracketed paste、resize、signal/termios restore、stable-row commit-once 和 search overlay。TUI 继续由单一 UI 线程投影和写屏，worker 只分发 typed command；新的 `NativeScrollbackRenderer` 在 normal screen 中只提交 settled transcript，provisional/activity/composer 留在 mutable tail。主屏不进入 alternate screen、不启用鼠标捕获、不发 ED3，因此 wheel/trackpad 直接使用终端原生 scrollback；只有 `Ctrl-F` 搜索成对借用 1049/1007。完整 async command bus、并发 turn、remote UI、OS sandbox 和并发/写入 subagent 仍未实现。
...
T-259 已完成 Semantic Coding Tools Phase 2。Preview 只生成 session-scoped opaque plan；apply 不重新请求 LSP，只消费该 plan 并重验 workspace/project/server identity 与每个文件 before hash。WorkspaceEdit parser、plan store、existing-file transaction、patch journal 和 approval 各自只有一个 Owner；多文件同步失败会补偿，journal 失败会恢复并保留可重试 plan，最终状态由磁盘 residual 复读决定。真实 JDTLS 已通过 2 文件 rename、journal failure/retry、`javac`、rollback 和 fresh-client stale matrix；同一长寿命 client 在外部 apply/rollback 后缺少 `didChange` 的再次 rename 仍是显式残余，不通过 watcher、自动重启或第二 lifecycle 掩盖。
