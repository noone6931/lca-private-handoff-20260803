...
当前 deterministic gate 为 1109/1109 tests、62/62 benchmark、22/22 architecture checks；T-232 stable release 为 `20260717T111651Z-b30240a91398-5bf520393764`，revision `b30240a91398db707b513c54db50eef65cffbef7`，digest `5bf5203937646ae6a2791a7fe820972b88312139c95647f8fc436802674be7af`。publish gate 在 clean detached candidate 中用 Python 3.14 再次通过 unittest、compileall 和 diff-check；独立 immutable matrix 另验证 repeated-stop A+B obligation、no-diff fail-closed、完整 verification closure、外部变化失效、telemetry redaction 和单一 Turn lifecycle。该版本继承 T-228 的 semantic preview/jdtls 边界，并补齐同步 session task continuity。async queue、取消、OS sandbox、并发/写入 subagent、worker/runtime 隔离与完整 TUI 仍未实现，不冒充追平 Codex/OMP。
