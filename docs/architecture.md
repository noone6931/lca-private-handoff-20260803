...
| Isolated read-only reviewer / ExploreHandoff | `[MVP-已落地，live provider 待收敛]` | `explore_handoff.py`、`read_only_reviewer.py`、`runtime_read_only_review.py`、`read_only_explore.py`、`runtime_read_only_explore.py`、`steering/pre_review.py`、`task_contract.py`、RunSummary。 | 高风险只读 owner/impact/design contract 先产生 typed profile。T-154 将 reviewer 改为唯一的 output-only `submit_read_only_review` function schema；它不是 ToolRegistry 工作区工具，不经审批、不执行文件或命令。T-155 的 pre-review coordinator 在 bounded explore 结束后合并同一候选的多个 hard evidence finding，一次重写覆盖全部类别；同一 evidence revision 最多两轮，耗尽时 truthful `unverified`，不会重开 discovery。随后才进入 reviewer；reviewer pass 后不再允许 presentation steerer 改写事实。 | 借鉴 OMP `tool-choice-queue.ts` 的 queued/in-flight/resolved/rejected directive 生命周期与 `agent-loop.ts` 的 turn-boundary 收束。owner/design profile 为降低 provider 参数混淆，仅在该阶段投影 `read_file`/`search_code` 并按最少观察 root 引导；这不是 LSP capability 不可用的判断，普通代码证据任务仍保留 LSP。LCA 不复制 task platform、也没有 OMP 同名 coordinator；output tool 不进入 workspace tool registry，严格失败不会被截断或默许。 |
