...
当前 deterministic gate 为 1195/1195 tests、62/62 benchmark、25/25 architecture checks；T-242 stable release 为 `20260718T033001Z-cd235cf3b47c-2076f2cbe834`，revision `cd235cf3b47cceae69b821466b9e41a3a433c4a9`，digest `2076f2cbe8342b1f8aa5170103d29ce878b9b62017b549792df66ade181b16b7`。publish gate 在 clean detached candidate 中用 Python 3.14 再次通过 unittest、compileall 和 diff-check；独立 matrix 还覆盖普通 coding continuity、semantic preview/jdtls 边界、full-screen TUI reducer/mailbox/interaction/cooperative cancellation、PTY restore/signal、`/workspace list` completion，以及 bracketed paste、wrapped-row scrollback 和真实 X10 wheel bytes。TUI 继续由单一 UI 线程投影和写屏，worker 只分发 typed command。T-242 将滚轮步长改为视口比例；空 composer 的 Up/Down 可直接滚历史，End 与新提交返回 live tail；底部不再展示 history/PageUp 教程，只有离开最新位置时才显示范围和返回提示。terminal mode Owner 参照 Codex alternate-scroll fallback 成对管理 DECSET 1007，并在正常退出、异常、信号和 suspend/resume 中恢复。当前 curses full-screen 仍使用内部 viewport；Codex/OMP 式 normal-screen native scrollback 需要独立 renderer 替换，未在本批冒充完成。完整 async command bus、并发 turn、remote UI、OS sandbox 和并发/写入 subagent 仍未实现。
