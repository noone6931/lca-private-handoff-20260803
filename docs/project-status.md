...
| T-151 | Isolated Read-only ExploreHandoff / Reviewer MVP | 已完成代码与离线门禁，待小红 live review | P12 | T-150 live S2 将通用支付/费用能力称为“已证实 owner”，S3 又编造既有表、类、审批和编号规则；确定性 benchmark 只喂正确答复无法发现。 | `explore_handoff.py` 从既有 EvidenceLedger/ToolResult 生成有界、root-aware claim matrix；`read_only_reviewer.py` 用独立 `system + user` 上下文、`tools=[]`、严格 JSON schema 检查候选答复。contract 先给出 typed owner/impact/design review profile；每 run 最多一次 reviewer call 与一次 `tools=[]` rewrite。reviewer 的精确 unsupported claim span 若不属于原 candidate 或在改写中原样回流，则 truthful `read_only_reviewer_unverified`，不进入 ping-pong；timeout 与 provider protocol violation 单列 telemetry。新增 benchmark 13/14；585 tests、offline benchmark 14/14。 |
...
| R-075 | 高风险只读分析将弱相关能力或设计建议伪装为已证实当前实现 | T-151 已缓解，待 live review | S2 把同领域支付类/前端能力说成真实 owner；S3 编造既有 DTO、编号、审批和退款复用。 | 不在 CompletionAudit 叠关键词：独立 reviewer 仅消费 bounded evidence handoff，并区分 direct binding、observed candidate、unlocated、requirement fact 与 proposal。无显式行为-to-symbol/path/call-chain 绑定的能力只能作为候选；reviewer 无效/超时/协议违规、无剩余预算或改写不合规则诚实未验证。 |
