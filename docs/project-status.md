...
当前项目已完成 P8 前端协议与交互基础 MVP，P9 真实需求使用准备已覆盖项目边界、源码验证、LSP 韧性和服务费结算链路压测，并进入 P10/P11 Intelligence Runtime 与真实跨项目任务收束。用户可以用自然语言描述任务，而不是每次手写工具顺序；Runtime 通过 RequirementContract、Planner/Explore、MiniToolChoiceQueue、CompletionAudit、二阶段 Patch Reviewer、VerificationPlan/TestPlanner/DeliveryAudit、bounded finalization coordinator、T-144 session evidence continuity、T-145 claim taxonomy、T-146 provider protocol boundary 与 T-147 intent/evidence closure 保持任务目标、证据、写入、测试和 diff 的闭环。T-147 阻止“未检查却声称检查过”、语义任务被强制读仓库，以及 Git metadata 被错误要求代码证据。下一步仍是在已确认的真实 owner 边界内做明确小改；完整 async Command Bus/streaming follow-up queue 仍后置。详见 `docs/pressure-test-2026-07-11.md`。
...
| P10 | Intelligence Runtime 骨架 | 已完成，进入 P11 稳定收束 | 按 OMP 架构原则补单 Agent 内部的目标契约、工具选择队列、完成审计、两阶段计划和 reviewer。T-123~T-148 已完成 ToolLoop registry、RunCollector/RunContext、最终 steering 拆分和 runtime phase owner；`agent.py` 已从 3,638 行/113 methods 降至 1,837 行/74 methods。 |
