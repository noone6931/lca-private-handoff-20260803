...
| T-147 | Release-blocker evidence intent closure | 已完成；待独立 review | P11 | stable 样本出现无工具却声称“已检查/未发现 Java”、纯语义解释被源码关键词拉入工具循环、以及 primary `git_status` non-repo probe 被 CompletionAudit 错当缺代码证据；review 又发现 metadata classifier 可抢占实现任务、probe root 与最终 Git 结论极性未审计。 | `negative_evidence.py` 将 bare observed Java 归为需真实 discovery 的 `observed_no_match`，但不会把经验/依赖/版本当文件结论；ToolUsageEvidence 识别已调用/使用/检查工具的伪证据并排除明确 no-run/建议语；RequirementContract 的 inspection-forbidden semantic-only contract 不开放仓库工具且要求真实语义回答。Git metadata 仅服务于直接 primary 状态问句，严格要求 `evidence_root_label=primary`，并把 structured `git_repository` True/False 与最终“是/不是”结论逐项核对。T-146 同时收紧：已识别完整、未围栏的百炼 XML tool envelope 在普通 phase 也不会静默展示，而是 `provider_protocol_violation`；fenced XML 与正常 structured tool phase 保持原义。542 tests、offline benchmark 11/11。提交 `aa48303`、`cea089e`、`ab097ea`、`c682b7b`。 |
