...
| T-147 | Release-blocker evidence intent closure | 已完成；待独立 review | P11 | stable 样本出现无工具却声称“已检查/未发现 Java”、纯语义解释被源码关键词拉入工具循环、以及 primary `git_status` non-repo probe 被 CompletionAudit 错当缺代码证据。 | `negative_evidence.py` 将 bare observed Java 归为需真实 discovery 的 `observed_no_match`，但不会把经验/依赖/版本当文件结论；ToolUsageEvidence 识别已调用/使用/检查工具的伪证据并排除明确 no-run/建议语；RequirementContract 增加 inspection-forbidden semantic-only contract，schema 与审计均不再开放仓库工具；Git-specific primary metadata 由结构化 `git_status` negative probe 单独验收。T-146 同时收紧：已识别完整、未围栏的百炼 XML tool envelope 在普通 phase 也不会静默展示，而是 `provider_protocol_violation`；fenced XML 与正常 structured tool phase 保持原义。538 tests、offline benchmark 11/11。提交 `aa48303`、`cea089e`、`ab097ea`。 |
