...
| T-145 | Epistemic Claim Taxonomy / Negative Evidence Stance | 已完成；live 有 provider 收尾残余 | P11 | 小牙 stable 样本把“未发现 Java，但不等于证明不存在”误判为绝对缺失，重新开放 glob 并拉长最终收束。 | `negative_evidence.py` 统一解析 `asserted_absence`、`observed_no_match`、`epistemically_qualified`、`quoted_or_hypothetical`，包含 clause span、scope、support requirement；只有绝对缺失进入完整 discovery evidence gate。RunSummary/session 记录 stance/blocked/qualified 指标；新增 benchmark 09。510 tests、offline benchmark 9/9。百炼临时 fixture session `20260712T020107510742Z` / run `dd5a3da31f654088b5787f90db4cfce6` 正确记录 `observed_no_match=1`、未触发 `negative_existence`，但 provider 额外浏览并在 CompletionAudit forced-final 输出原始 XML tool-call 文本，live strict acceptance 0/1；作为 T-146 候选记录，非 taxonomy 回归。提交 `2009da8`、`8fd89ae`、`b0798bc`。 |
...
| R-072 | 谨慎的负向观察被 Runtime 当成绝对缺失 | 已关闭；持续观察 provider 收尾质量 | 元讨论、引用或“未发现但不证明”曾被关键词审计升级为缺失断言，导致多余 glob/finalization。 | T-145 用统一 taxonomy 和 clause-local stance 取代各消费者的短语扫描；绝对缺失仍需 scope-complete discovery，observed/qualified 不补搜。live 已确认不会触发 negative steerer；provider 仍可能自行额外浏览或在 forced-final 输出伪 tool markup，作为后续 T-146 收尾可靠性候选。 |
