...
| T-149 | Finalization Budget + Git Conclusion Ownership Closure | 已完成；待独立 review | P11 | stable session `20260712T050100282874Z` 的 primary non-repo probe 与正确草稿被“其他路径需要检查是否为 Git 仓库”误判成正向 Git 结论；25 秒预算也被固定 45 秒 reserve 从起点禁用 hard rewrite。 | `completion_audit.py` 以 primary subject 的 clause-local declarative conclusion 审计 Git 极性；先归一受限 Markdown 展示（加粗、行内路径），疑问、条件和 additional-root 未决说明不参与。`finalization.py` / `run_context.py` 按运行总 budget 计算 effective reserve（25 秒任务为 5 秒），RunContext 统一提供 queue/skip decision，真正临近 deadline 仍保留 unverified hard gate。558 tests、offline benchmark 11/11；新增 deterministic 25 秒 `git_status -> tools=[] rewrite -> final` 回归，Case3 已关闭。 |
