...
| P10 | Intelligence Runtime 骨架 | 已完成，进入 P11/P12 稳定收束 | 按 OMP 架构原则补单 Agent 内部的目标契约、工具选择队列、完成审计、两阶段计划和 reviewer。T-123~T-153 已完成 ToolLoop registry、RunCollector/RunContext、最终 steering 拆分、runtime phase owner、临时工具 directive、document-only 证据域和 isolated read-only reviewer；`agent.py` 当前 1,909 行/74 methods。 |
| P12 | Read-only Convergence Closure | T-150~T-153 离线完成；live 语义收敛继续观察 | 临时工具限制已变为有界 lifecycle，document-only 需求分析与源码调查已拆成 typed evidence domain；T-152 让 reviewer schema 有界 repair、owner/design explore 进入有限精确证据预算，T-153 进一步按实际执行工具数截断同批 batch 并优先直接读取 typed 候选路径。命名 session 仅在 read policy 预授权时安全恢复经磁盘 hash 复核的正向 read evidence。 |
