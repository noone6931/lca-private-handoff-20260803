...
| P10 | Intelligence Runtime 骨架 | 已完成，进入 P11 稳定收束 | 按 OMP 架构原则补单 Agent 内部的目标契约、工具选择队列、完成审计、两阶段计划和 reviewer。T-123~T-148 已完成 ToolLoop registry、RunCollector/RunContext、最终 steering 拆分和 runtime phase owner；`agent.py` 已从 3,638 行/113 methods 降至 1,852 行/75 methods。 |
...
| R-054 | `agent.py` steering/guard 继续膨胀 | T-148 已关闭，连续不回涨 1/3 | review 指出 compaction 已拆出，但 final/read-only/semantic guard 继续堆在 `agent.py`。 | T-148 已将 final steering、provider context、workspace/session roots、evidence/verification/session cache、memory consolidation 迁入职责 owner；主文件从 3,638 行/113 methods 降至 1,852 行/75 methods。architecture boundary tests 锁定 `<=2,100` 行、`<=75` methods 与 helper 不回流；后续三个连续变更批次继续记录不回涨。 |
