...
| P10 | Intelligence Runtime 骨架 | 已完成，进入 P11/P12 稳定收束 | 按 OMP 架构原则补单 Agent 内部的目标契约、工具选择队列、完成审计、两阶段计划和 reviewer。T-123~T-156 已完成 ToolLoop registry、RunCollector/RunContext、最终 steering 拆分、runtime phase owner、临时工具 directive、document-only 证据域、isolated read-only reviewer、pre-review audit coordinator、图片观察与安全部分交付；`agent.py` 当前 1,948 行/74 methods。 |
| P12 | Read-only Convergence Closure | T-150~T-156 离线完成；live provider 收敛继续观察 | 临时工具限制已变为有界 lifecycle，document-only 需求分析与源码调查已拆成 typed evidence domain；T-154 将 reviewer 收束为 output-only typed yield，T-155 把 bounded explore 后同一候选的多个 hard audit 合并、按 evidence revision 最多两轮收敛，T-156 在 reviewer non-pass 时只输出 typed evidence handoff 的安全部分报告。命名 session 仅在 read policy 预授权时安全恢复经磁盘 hash 复核的正向 read evidence。 |
