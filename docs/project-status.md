...
| P10 | Intelligence Runtime 骨架 | 已完成，进入 P11/P12 稳定收束 | 按 OMP 架构原则补单 Agent 内部的目标契约、工具选择队列、完成审计、两阶段计划和 reviewer。T-123~T-150 已完成 ToolLoop registry、RunCollector/RunContext、最终 steering 拆分、runtime phase owner、临时工具 directive 与 document-only 证据域；`agent.py` 保持 1,900 行以内/74 methods。 |
| P12 | Read-only Convergence Closure | 已完成，待 stable review | 将临时工具限制变为有界 lifecycle，并将 document-only 需求分析与源码调查拆成 typed evidence domain；下一步用真实但非企业源码 fixture 观察 provider 工具选择效率。 |
...
| T-150 | Read-only Convergence Closure | 已完成，待 stable review | P12 | stable 多 root 样本的 `negative_existence` 临时 allowlist 未消费，导致 66 次 `glob_files`、79 次 compaction 后超时；document-only 需求分析也被按源码推断合同反复审计。 | 对齐 OMP `tool-choice-queue.ts` 的 directive resolve/reject 生命周期与 `agent-loop.ts` 的 active-tool turn boundary：`TemporaryToolDirectiveOwner` 以 source-level attempt/turn budget 管理 temporary schema，成功、错误、拒绝与耗尽均可审计，耗尽后只排一次 `tools=[]` truthful final；`document_only` evidence domain 仅暴露 `list_files/read_file/ask_user`，CompletionAudit 只承认 Markdown/HTML evidence 并拒绝实际代码检查绕路。新增 benchmark 12；571 tests、offline benchmark 12/12。 |
