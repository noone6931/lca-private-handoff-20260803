...
| P10 | Intelligence Runtime 骨架 | 已完成，进入 P11/P12 稳定收束 | 按 OMP 架构原则补单 Agent 内部的目标契约、工具选择队列、完成审计、两阶段计划和 reviewer。T-123~T-151 已完成 ToolLoop registry、RunCollector/RunContext、最终 steering 拆分、runtime phase owner、临时工具 directive、document-only 证据域和 isolated read-only reviewer；`agent.py` 保持 1,900 行以内/74 methods。 |
| P12 | Read-only Convergence Closure | T-150 已完成；T-151 实现完成、live provider 兼容性待收束 | 临时工具限制已变为有界 lifecycle，document-only 需求分析与源码调查已拆成 typed evidence domain；T-151 再以隔离 reviewer 约束 owner/影响范围/设计草案的语义断言，命名 session 可安全恢复经磁盘 hash 复核的正向 read evidence。 |
