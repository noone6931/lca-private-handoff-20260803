...
| P12 | Read-only Convergence Closure | T-150 已完成；T-151 待独立 live review | 临时工具限制已变为有界 lifecycle，document-only 需求分析与源码调查已拆成 typed evidence domain；T-151 再以隔离 reviewer 约束 owner/影响范围/设计草案的语义断言。 |
...
| T-151 | Isolated Read-only ExploreHandoff / Reviewer MVP | claim-ID 代码与离线门禁完成，待小红 live review | P12 | T-150 live S2 将通用支付/费用能力称为“已证实 owner”，S3 又编造既有表、类、审批和编号规则；确定性 benchmark 只喂正确答复无法发现。 | `explore_handoff.py` 从既有 EvidenceLedger/ToolResult 生成有界、root-aware claim matrix；`read_only_reviewer.py` 用独立 `system + user` 上下文、`tools=[]`、严格 JSON schema 检查候选答复。contract 先给出 typed owner/impact/design review profile；候选先切成稳定 `claim_id` 单元并 head/tail 均衡采样，reviewer finding 必须指向唯一 ID。每 run 最多一次 reviewer call 与一次 `tools=[]` rewrite；原 claim unit Markdown-normalized exact 回流才 `read_only_reviewer_unverified`。合法 `unverified` finding 会先排一次诚实降级改写，未知 ID、timeout、协议违规、预算不足或回流仍终止；RunSummary 仅记录 reviewed-claim count。新增 benchmark 13/14；588 tests、offline benchmark 14/14。 |
