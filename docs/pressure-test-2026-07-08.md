...
| 企业项目联网 LCA 压测 | 用户本机已跑通链路，重复搜索硬停已改善，但 multi-root 需求读取仍需加强 | Codex 宿主仍不能代跑企业源码/需求外发。用户在本机允许环境运行 session `20260708T062614211387Z`，成功使用多种只读工具，但围绕 `feePlan` 重复搜索后硬停、无最终分析。提交 `d140199` 后复跑 session `20260708T065705459243Z`，已能输出最终分析且未再重复硬停；但它没有读取 `--allow-dir` 需求目录。提交 `502362a` 后复跑 session `20260708T070722601499Z`，仍先猜 `requirements`，最终靠重复工具 steering 输出代码侧结论，说明仅 system prompt 注入 roots 不够，需要在工具观察里持续暴露 roots。 |
...
当前 Codex 宿主不能代跑企业源码/需求外发到百炼。用户在允许环境中可直接运行：
...
| PT-011 | P0 | `--allow-dir` 的具体路径没有稳定进入模型操作路径，导致模型猜错需求目录。 | session `20260708T065705459243Z` 和 `20260708T070722601499Z` 都传入 `--allow-dir /Users/chengming/mynote/1_projects/0630_YXR-971_平台通用优化`，但模型仍先调用 `list_files {"path":"requirements"}` 并得到 path not found，后续未读取真实需求文档。 | OMP 会把 cwd、project context、rules 和工具上下文作为运行时环境显式提供；多 root 场景下，模型不能只知道“有 allowed dir”，还要在工具观察里持续看到可访问根，否则容易回到惯性目录名。 | 已补第二版：system prompt/provider-bound context 保留 `[Workspace roots]`；`list_files {}` 根目录输出会显示 primary workspace 和 allowed dirs；`list_files` / `search_code` path-not-found、带 allowed-dir 的空搜索会提示 exact allowed-dir 路径。 |
...
| 2 | 用户本机复跑同一企业需求命令，重点看是否先读取 allowed-dir 需求文档 | 新修复会把 allowed dirs 同时暴露在 system prompt 和工具观察里；下一次即使先猜 `requirements`，path-not-found 也应把模型拉回真实需求目录。 |
