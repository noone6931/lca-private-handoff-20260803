...
| 2 | 增加 OMP 风格 `--state-dir` | 避免只读压测或跨项目分析在目标仓库写 `.local-agent/sessions`，让 runtime 状态与项目源码分层。 |
| 3 | 评估 OMP 风格 ToolChoiceQueue / soft tool requirement | 只有当真实任务仍出现“该用某工具但不用/反复偏航”时再引入更重 runtime steering。 |
| 4 | 在允许环境中复测企业需求链路 | 当前 Codex 执行环境不能代跑真实企业代码/需求外发；用户本机 LCA 可按用户/provider/permission 策略执行。 |
