...
def _java_diff(*, public_api: bool = False) -> str:
...
class PatchReviewerTests(unittest.TestCase):
    def test_raw_test_intent_never_creates_a_deterministic_finding(self) -> None:
        requests = (
            "请实现解析器并补充测试。",
            "Fix the parser. Do not rewrite tests.",
            "修复解析器，无需修改或新增测试。",
        )

        for request in requests:
            with self.subTest(request=request):
                result = _review_request(request)
    ...
    def test_large_diff_metadata_preserves_late_public_api_symbols_for_reviewer(self) -> None:
        request = "请实现用户名规范化。"
        source_only = _java_diff().split("\n[diff summary]", 1)[0]
        public_api = _java_diff(public_api=True).split("\n[diff summary]", 1)[0]
        ...
        self.assertNotIn("public String normalize", summary)
        self.assertIn("call_site_review_missing", {finding.code for finding in result.findings})


def _review_request(request: str):
