...
- For owner/design profiles, every candidate statement presented as a current repository/source fact must be bound by a claim-scoped `source_locator`, or be directly and unambiguously supported by a supplied generic source item. A bare path, symbol name, or assertion that files were read is not evidence for the claimed behavior. When that binding is absent, report the claim as unsupported evidence scope; this proves an answer defect without asserting that the repository fact itself is false. Clearly labeled requirement facts, proposals, and open questions are not current-source claims and do not require `source_locator` support.
